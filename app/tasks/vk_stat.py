...
async def handler():
    async with httpx.AsyncClient() as client:
        get_gos_bage = await client.post(
            f"https://{settings.DOMAIN}/api/vk/get_gos_bage", timeout=6000
        )
        print(f"/vk/get_gos_bage: {get_gos_bage.status_code}")

        wall_get_all = await client.post(
            f"https://{settings.DOMAIN}/api/vk/wall_get_all", timeout=6000
        )
        print(f"/vk/wall_get_all: {wall_get_all.status_code}")

        wall_get_all = await client.post(
            f"https://{settings.DOMAIN}/api/vk/get_group_data", timeout=6000
        )
        print(f"/vk/wall_get_all: {wall_get_all.status_code}")

        get_weekly_audience_reach = await client.post(
            f"https://{settings.DOMAIN}/api/vk/get_weekly_audience_reach", timeout=6000
        )
